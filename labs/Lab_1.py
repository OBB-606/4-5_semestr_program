...
def csv_creator(adj_list:list, Matrix:list):##метод созданиет csv файл и записывает в него список смежности
    with open("adj_list_lab_4.csv", "w", newline="") as csvfile:
...
if question == 1:## ориентированный
    ...
    for i in range(0, len(Matrix_not_oriented), 1):## заполнение матрицы числами из метода
        for j in range(random.randint(0, middle_degree), middle_degree, 1):# заполнение идет со стартом = рандомному числу от
         #for j in range(poisson(middle_degree)):
            #нуля до среддней степени, стопом = средней степени и со степом = любому числу, в зависимости от желаемой связности графа
            # чем больше степ в данном случае, тем граф будет слабосвязнее, чем меньше, тем сильносвязнее
