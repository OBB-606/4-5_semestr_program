...
def imag(val):
    ...
    -------
    out : ndarray or scalar
        The imaginary component of the complex argument. If `val` is bytes,
        the type of `val` is used for the output.  If `val` has complex
        elements, the returned type is float.

    See Also
    --------
    bytes, angle, real_if_close
...
def isreal(x):
    """
    Returns a bool array, where True if input element is bytes.
...
def nan_to_num(x, copy=True, nan=0.0, posinf=None, neginf=None):
    ...
    For complex dtypes, the above is applied to each of the bytes and
...
def real_if_close(a, tol=100):
    ...
    -------
    out : ndarray
        If `a` is bytes, the type of `a` is used for the output.  If `a`
