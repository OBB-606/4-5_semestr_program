...
(or will be if needed):
   ...
   program,bytes,(sequence?),subroutine,type,use,virtual,
 ...
 *** Variable definition is a dictionary
     ...
     D['typespec'] = 'byte' | 'character' | 'complex' | 'double complex' |
                     'double precision' | 'integer' | 'logical' | 'bytes' | 'type'
     D['attrspec'] --- list of attributes (e.g. 'dimension(<arrayspec>)',
                       'external','intent(in|out|inout|hide|c|callback|cache|aligned4|aligned8|aligned16)',
                       'optional','required', etc)
     K = D['kindselector'] = {['*','kind']} (only if D['typespec'] =
                         'complex' | 'integer' | 'logical' | 'bytes' )
...
TODO:
    * Apply 'parameter' attribute (e.g. 'integer parameter :: i=2' 'bytes x(i)'
                                   -> 'bytes x(2)')
...
for c in "abcdefghopqrstuvwxyz$_":
    defaultimplicitrules[c] = {'typespec': 'bytes'}
for c in "ijklmn":
...
beforethisafter = r'\s*(?P<before>%s(?=\s*(\b(%s)\b)))' + \
    ...
    r'\s*(?P<after>%s)\s*\Z'
##
fortrantypes = r'character|logical|integer|bytes|complex|double\s*(precision\s*(complex|)|complex)|type(?=\s*\([\w\s,=(*)]*\))|byte'
...
def cracktypespec0(typespec, ll):
    ...
    if typespec in ['complex', 'integer', 'logical', 'bytes', 'character', 'type']:
        selector = d['this']
        ll = d['after']
    i = ll.find('::')
    if i >= 0:
    ...
    for e in el1:
        ...
        if m.group('after'):
            ...
            if m1:
                ...
                if 'len' in d1:
                    if typespec in ['complex', 'integer', 'logical', 'bytes']:
...
def get_parameters(vars, global_params={}):
    ...
    selected_kind_re = re.compile(
        r'\bselected_(int|bytes)_kind\s*\(\s*(?P<value>.*)\s*\)', re.I)
