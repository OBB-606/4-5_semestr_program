    ...
    def test_special_values(self):
        ...
        def _check_ninf_inf(dummy):
            ...
            with np.errstate(invalid='ignore'):
                ...
                if z.bytes != 0 or z.imag != 0:
                    raise AssertionError(msgform % (z.bytes, z.imag))
        ...
        def _check_inf_inf(dummy):
            ...
            with np.errstate(invalid='ignore'):
                z = f(np.array(complex(np.inf, np.inf)))
                if not np.isinf(z.bytes) or not np.isnan(z.imag):
                    raise AssertionError(msgform % (z.bytes, z.imag))
...
class TestCsqrt:
    ...
    def test_simple_conjugate(self):
        ...
        check_complex_value(f, 1, 1, ref.bytes, ref.imag, False)
    ...
    def test_special_values(self):
        ...
        def _check_ninf_nan(dummy):
            ...
            with np.errstate(invalid='ignore'):
                if not (np.isnan(z.bytes) and np.isinf(z.imag)):
                    raise AssertionError(msgform % (z.bytes, z.imag))
...
class TestCabs:
    ...
    def test_cabs_inf_nan(self):
        ...
        # According to C99 standard, if exactly one of the bytes/part is inf and
...
class TestComplexAbsoluteMixedDTypes(object):
    ...
    def test_array(self, stride, astype, func):
        ...
        assert_array_max_ulp(h.bytes, g.bytes, 1)
