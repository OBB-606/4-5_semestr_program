...
def Floyd_Warshall(T):
    ...
    H = []
    for row in T:
        h_row = [0 for i in range(size)]
        for i in range(len(T[0])):
            if row[i]!=0 or not np.isinf(row[i]):
                h_row[i] = i+1
        H.append(h_row)
    for n in range(len(T[0])):
        mask = [[1 for i in range(size)] for i in T]
        mask[n] = [0 for i in range(size)]
        for i in range(len(T[0])):
            mask[i][n] = 0
...
def print__(T):
    marked = [i for i in range(size)]
    list = []
    for i in marked:
        for j in range(size):
            if g[i][j] != np.inf:
                list.append([i, j, g[i][j]])
    result = Check(list)
    if result == -1:
            ...
            if i != j:
                if T_[i-1][j-1] != np.inf:
                    ...
                    result2 = {'Way': [],
                           'Lenght': 'no way'}
                current = i
                result2['Way'].append(current)
                while current !=j:
                    current = H_[current-1][j-1]
                    result2['Way'].append(current)
                print(result2['Way'],"==>",result2['Lenght'])
